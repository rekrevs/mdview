...
Initial attempt used NSView keyDown handling - failed on non-US keyboards.

Fixed implementation uses SwiftUI's menu system:
- mdviewApp.swift: CommandGroup(after: .toolbar) with .keyboardShortcut()
- ContentView.swift: Listens via NotificationCenter for zoom events
- Proper keyboard localization handled by macOS automatically
- Base font 14pt (matching MarkdownUI default)
